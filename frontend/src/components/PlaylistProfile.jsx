...
const PlaylistProfile = () => {
  ...
  if (isLoading) {
    return <div className="p-4 bg-base-200 flex justify-center items-center h-screen">Loading...</div>;
  }
          ...
          <div className="card bg-base-100 shadow-xl">
            <div className="card-body items-center text-center">
              ...
              <div className="card-actions justify-center mt-4">
                <button className="btn btn-primary" onClick={() => setIsCreatePlaylistOpen(true)}>Create Playlist</button>
              </div>
            </div>
          </div>
        ) : (
            ...
            {playlists.map((playlist) => (
              <div key={playlist?.id} className="card bg-base-100 shadow-xl">
                <div className="card-body p-4">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="avatar placeholder flex items-center justify-center">
                        <div className="bg-primary text-primary-content rounded-lg w-12 items-center" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <BookOpen size={24} />
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1 text-sm text-base-content/70">
                          ...
                          <div className="flex items-center gap-1">
                            <Clock size={14} />
                            <span>Created {playlist?.createdAt ? formatDate(playlist.createdAt) : 'Unknown date'}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  {/* Description */}
                  <p className="text-base-content/80 mt-1">{playlist?.description || 'No description provided.'}</p>
                            ...
                            <tbody>
                              {playlist?.problems?.map((item) => (
                                <tr key={item?.id} className="hover">
                                  <td className="font-medium">{item?.problem?.title || 'Untitled Problem'}</td>
